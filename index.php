...
try{
    ...
    if($hasNDriveFolder){
        ...
        // add backup file to one drive (using ndrive local folder)
        $ndrive = dirname(__FILE__) . '/vidikhan/';
        $backup = dirname(__FILE__) . '/backup-' .$fileName . '.zip';
        copy($backup, $ndrive . $fileName . '.zip');
        ...
        $files = glob(dirname(__FILE__) . '/vidikhan/*'); // get all file names
        foreach($files as $file){ // iterate files
            // dd($files, $file, time() - filemtime($file));
